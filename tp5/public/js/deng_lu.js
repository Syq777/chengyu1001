...
/*	$('input').eq(0).focus(function(){
		if($(this).val().length==0){
			$(this).parent().next("div").text("请输入QQ号或者手机号");
		}
	})
...
/*
	$('input').eq(0).blur(function(){
		if($(this).val().length==0){
			...
			$(this).parent().next("div").text("只能是10-11个数字");
...
//	确认密码
	$('input').eq(2).blur(function(){
		if($(this).val().length==0){
			...
			$(this).parent().next("div").text("两次密码不一样");
			$(this).parent().next("div").css("color",'red');
		}else{
			$(this).parent().next("div").text("");
		}		
...
//	 验证码刷新
	function code(){
		var str="=abcdefghjklmn1234567890QWERTYUIOPLKJHGFDSAZXCVBNM";
		var str1=0;
		for(var i=0; i<4;i++){
...
//	提交按钮
	$("#submit_btn").click(function(e){		
		...
		//协议
		/*if($("#xieyi")[0].checked){
			//向变量stuList数组添加一个数值，数值内部格式Student(name,password,id)
			//发送用户信息
            ...
            alert("注册成功");
			window.open("wan_shan.html","_blank");
		}else{						
